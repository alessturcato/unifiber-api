...
use App\Models\CopertureUnifiberTest;
use App\Models\User;
...
class CopertureUnifiberTestController extends Controller
{
    // Nome del file di log
    protected $log_name= 'coverage_request_test';
    ...
    public function ricerca(Request $request)
    {
        ...
        try {
            ...
            $result= CopertureUnifiberTest::where($query_conditions)->paginate($limit)->toArray();
            unset($result['links']);
    ...
    public function registra(Request $request)
    {
        ...
        try {
            ...
            $result= CopertureUnifiberTest::where($query_conditions)->first();

            if(empty($result))
            {
                Log::channel($this->log_name)->info($id_notifica.'Richiesto inserimento nuova copertura');
                $result= CopertureUnifiberTest::create($request->all());
    ...
    public function storico($id, Request $request)
    {
        ...
        try {
            $results= CopertureUnifiberTest::findOrFail($id)->versions->toArray();
            ...
            Log::channel($this->log_name)->info($id_notifica.'Storico recuperato correttamente. Copertura: '.$id.'');
